...
export default class Cl_index {
  ...
  iniciarData() {
    this.controlador.procesarPaqueteNacional(
      new Cl_mNacional(paqueteNacional[0])
    );
    this.controlador.procesarPaqueteNacional(
      new Cl_mNacional(paqueteNacional[1])
    );
    this.controlador.procesarPaqueteNacional(
      new Cl_mNacional(paqueteNacional[2])
    );
    this.controlador.procesarPaqueteNacional(
      new Cl_mNacional(paqueteNacional[3])
    );

    this.controlador.procesarPaqueteInternacional(
      new Cl_mInternacional(paqueteInternacional[0])
    );
    this.controlador.procesarPaqueteInternacional(
      new Cl_mInternacional(paqueteInternacional[1])
    );
    this.controlador.procesarPaqueteInternacional(
      new Cl_mInternacional(paqueteInternacional[2])
    );
    this.controlador.procesarPaqueteInternacional(
      new Cl_mInternacional(paqueteInternacional[3])
    );
  }
}
